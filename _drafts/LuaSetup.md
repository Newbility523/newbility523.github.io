...
虽然测试脚本只用到了 LuaJIT （LuaJIT 支持加载源码和 jit 字节码），但是 Luarocks 安装库的时候需要指定一个版本的 Lua 安装路径，所以也必须安装一个 Lua，LuaJIT 兼容至 Lua5.1，所以使选用 [Lua5.1.4](https://www.lua.org/ftp/lua-5.1.4.tar.gz) 。
...
进入 Lua 目录，**使用 gitbash 进行本节操作，cmd 会有一些语法不支持。**其实也能改 makefile，我懒得查了 。

执行命令

```shell
# 这里的指令都是一个操作，将 lua 编译安装。但是由于 window 或者 makefile 问题，尝试了多次，只有第一种可以成功编译。
# 成功
...
下载 [luarocks-3.9.2-windows-64.zip](https://luarocks.github.io/luarocks/releases/luarocks-3.9.2-windows-64.zip) 也放入 C 盘，并加入 Path 中，就算完成了安装。
...
如果一切正常，终端运行 luarocks 应该是这样的，路径会有点出入，但是都是`(ok)`

```bash
   Lua:
      Version    : 5.1
      Interpreter: C:/Lua/5.1\bin/lua.exe (ok)
      LUA_DIR    : C:/Lua/5.1 (ok)
      LUA_BINDIR : C:/Lua/5.1\bin (ok)
      LUA_INCDIR : C:/Lua/5.1/include (ok)
      LUA_LIBDIR : C:/Lua/5.1/bin (ok)

   Configuration files:
      System  : C:/Program Files/luarocks/config-5.1.lua (not found)
      User    : C:/Users/Administrator/AppData/Roaming/luarocks/config-5.1.lua
      (ok)
